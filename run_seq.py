...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        new_conv_in = torch.nn.Conv2d(12, unet_bg.conv_in.out_channels, unet_bg.conv_in.kernel_size, unet_bg.conv_in.stride, unet_bg.conv_in.padding)
        new_conv_in.weight.zero_()
        new_conv_in.weight[:, :4, :, :].copy_(unet_bg.conv_in.weight)
        new_conv_in.bias = unet_bg.conv_in.bias
        unet_bg.conv_in = new_conv_in
    
    unet_bg_original_forward = unet_bg.forward

    def hooked_unet_bg_forward(sample, timestep, encoder_hidden_states, **kwargs):
        c_concat = kwargs['cross_attention_kwargs']['concat_conds'].to(sample)
        c_concat = torch.cat([c_concat] * (sample.shape[0] // c_concat.shape[0]), dim=0)
        new_sample = torch.cat([sample, c_concat], dim=1)
        kwargs['cross_attention_kwargs'] = {}
        return unet_bg_original_forward(new_sample, timestep, encoder_hidden_states, **kwargs)
    
    unet_bg.forward = hooked_unet_bg_forward
    ...
    sd_offset = sf.load_file(model_path)
    sd_origin = unet_bg.state_dict()
    keys = sd_origin.keys()
    sd_merged = {k: sd_origin[k] + sd_offset[k] for k in sd_origin.keys()}
    unet_bg.load_state_dict(sd_merged, strict=True)
    del sd_offset, sd_origin, sd_merged, keys
    ...
    unet_bg = unet_bg.to(device=device, dtype=torch.float16)
    # rmbg = rmbg.to(device=device, dtype=torch.float32)

    # SDP

    unet.set_attn_processor(AttnProcessor2_0())
    unet_bg.set_attn_processor(AttnProcessor2_0())
    ...
    generator(config.input_path, config.generation.latents_path,
              config.generation.output_path, frame_ids=frame_ids, unet_bg=unet_bg)
